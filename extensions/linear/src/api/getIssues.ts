...
export async function isMilestoneComplete(milestoneId: string) {
  const { graphQLClient } = getLinearClient();
  const { data } = await graphQLClient.rawRequest<{ issues: { nodes: IssueResult[] } }, Record<string, unknown>>(
    `
      query($milestoneId: ID) {
        issues(filter: { projectMilestone: { id: { eq: $milestoneId }}, state:{type: {neq: "Done"}}  }) {
          nodes {
            ${IssueFragment}
          }
        }
  ...
  return data?.issues.nodes && data?.issues.nodes.length <= 0;
}
